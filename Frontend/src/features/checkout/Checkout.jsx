...
export default function Checkout() {
  ...
  // --- Extracted Calculations via Helper ---
        ...
        <div className="space-y-6">
          <section className="rounded-2xl border border-border bg-card p-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <Field label="Phone" value={form.phone} onChange={(v) => handleFormChange("phone", v)} placeholder="+1 555 0100" />
              <div className="md:col-span-2">
                <Field label="Delivery Address" value={form.address} onChange={(v) => handleFormChange("address", v)} placeholder="124 Market St, San Francisco, CA" required />
              </div>
            </div>
          </section>
...
function OrderSummaryPanel({ items, itemCount, subtotal, discount, shipping, total }) {
  return (
    <aside className="h-fit rounded-2xl border border-border bg-card p-6">
      ...
      <button type="submit" className="mt-6 w-full rounded-xl bg-foreground px-5 py-4 text-sm font-semibold text-background hover:bg-foreground/90 transition-colors">
        Finalize Order →
      </button>
      ...
      <Link to="/cart" className="mt-4 block text-center text-sm text-muted-foreground hover:text-foreground transition-colors">
        ← Continue shopping
      </Link>
    </aside>
  );
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        className="w-full rounded-lg border border-border bg-background px-3 py-2.5 text-sm outline-none focus:border-foreground transition-colors"
      />
    </label>
  );
}
