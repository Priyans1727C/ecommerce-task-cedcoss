...
export default function CartPage() {
  ...
  // --- Extracted Calculations via Shared Helper ---
  const { subtotal, discount, shipping, total } = calculateCartTotals(items, products);

  // --- Side Effects ---
  useEffect(() => {
    document.title = "Cart — BulkBuy";
    const metaDesc = document.querySelector('meta[name="description"]');
    if (metaDesc) {
      metaDesc.setAttribute("content", "Review your bulk order before checkout.");
    }
  }, []);

  // --- Early Return (Empty Cart State) ---
  if (items.length === 0) {
    return (
  ...
  // --- Handlers ---
  const handleUpdateQty = (productId, currentQty, step, minOrder) => {
    const newQty = Math.max(minOrder, currentQty + step);
    dispatch(updateCartItem({ id: productId, qty: newQty }));
  };
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 md:px-8">
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_380px]">
        ...
        {/* Right Column: Pricing Summary Panel */}
        <CartSummaryPanel 
          subtotal={subtotal} 
          discount={discount} 
          shipping={shipping} 
  ...
  return (
    <div className="flex gap-4 rounded-2xl border border-border bg-card p-4">
      ...
      <div className="flex flex-1 flex-col">
        ...
        {/* Quantity Selection & Line Calculations */}
        <div className="mt-auto flex items-end justify-between pt-3">
          <div className="flex items-center gap-1 rounded-full border border-border px-1">
            <button 
              onClick={() => onUpdateQty(item.productId, item.qty, -product.minOrder, product.minOrder)} 
